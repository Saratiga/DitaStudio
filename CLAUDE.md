...
- Плашкой в режиме «Автор» показывается только сам непереводимый лист (image,
  xref без текста, comment...) — раньше в плашку схлопывался весь фразовый элемент
  целиком, если нигде в его поддереве не было текста (`<b><image/></b>` было не
  видно и не редактируемо). `AppendChildren` в `InlineEditor.cs` теперь спускается
  в любой непустой фразовый элемент по числу детей, а не по наличию текста —
  каждый лист получает своё представление на любой глубине. Живая правка внутри
  самой плашки (например, alt-текст картинки) по-прежнему только через исходный код.
...
- Плагинной системы нет — расширение формата вывода или кастомное правило стиля
  требует правки кода и пересборки.
