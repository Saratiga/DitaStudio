...
dotnet run --project tests\DitaStudio.Tests   # 107 проверок ядра, код возврата 0 = всё прошло
...
(предпросмотр). Ядро — без зависимостей вообще, так и держать.
...
Внешние DTD не загружаются принципиально: словарь встроенный.
...
- `.ditaval` не читается; условия сборки задаются в диалоге «Условия сборки».
- Глубоко вложенные фразовые элементы в режиме «Автор» показываются плашками —
  тонкая правка через режим исходного кода.
